---
title: COSC349 Lab 2—Cloud Architecture—2025
tags: [cosc349, lab]
...
:::danger
:bomb: 
This lab exercise is still being updated, but you're welcome to give it a go anyway!
:::
...
When we discover bugs, typos, or areas that need elaboration in any of the lab exercises, Dave or Aaron may well be fixing things _in_ the lab class. If you log in to https://hackmd.io you will be able to edit the lab exercises yourself, and will have any changes anyone makes to this page pushed to your web browser dynamically. If you do not log in, before seeking help, try reloading the web page to receive any updates to its content.
:::
...
## Initial configuration of VirtualBox in the Owheo Labs

:::success
:pencil2:
Note that I am showing screen captures from my laptop, which is a macOS device with an Intel CPU running a recent version of VirtualBox 7 (7.0.8). The SoC Lab environment is running a similar version of VirtualBox (7.0.26). It may be that the SoC Lab version stays fixed as it is presently for the semester, even if new VirtualBox versions are released, so just be aware that you may end up with a slightly newer version of VirtualBox on your own laptop if you install it in future weeks.
:::

:::danger
:warning:
The backups for your School of Computing network home may not include your virtual machine disk images. Thus, you should be careful to ensure that you can easily restore the content of any VM disks that you create.
:::
...
:::warning
:warning: Older versions of VirtualBox had "Windows 2003 (32-bit)" instead of "Windows Server 2003 (32-bit)". They both refer to the same thing.
:::
...
:::danger
:bomb:
Bitnami is now owned by Broadcom, who have decided to stop making the Bitnami images avaliable for free. As such, the rest of the lab is now broken until we rewrite it to use a different OVA image.

...although we've located an old version of the file, which is now available in the `K:` drive of the Owheo Lab computers, specifically in the `K:\Coursework\COSC349` directory, filename `bitnami-lampstack-7.2.19-2-r56-linux-debian-9-x86_64.ova`. You can also [download it from OneDrive] if you're working on your own machine.
:::

[Bitnami]: https://bitnami.com
[Bitnami application catalog]: https://bitnami.com/stacks
[download it from OneDrive]: https://otagouni.sharepoint.com/:u:/s/COSC3492022/Eb7S04AkgptCqjHfBSHQtyYBE_CwpRwXbf6Dj3qyNjKWiA?e=FGpnGT
...
However before starting this VM, we need to reconfigure the network. Scroll the right hand pane down so that you see the "Network" section, and note that it is "Bridged Adaptor", which is the type of VirtualBox network that we should not use within the Owheo labs.
...
:::warning
:warning: 
However this will not work, as the particular IP address 10.0.2.15 here only exists "behind" the NAT mapping, so browsers on your host computer cannot connect to that address. (Originally I tried using a host-only network for this part of the lab, which was reasonably straightforward on my laptop, but did not work in the Owheo Labs.)
:::
...
There are many ways that VirtualBox can connect together the two VMs that you have created, using a virtual network. The simplest approach is to use a VirtualBox host-only network, but as noted, this did not work as expected in the Owheo labs.
...
However, we will try a different way to get the two VMs connected, that was introduced in VirtualBox version 5 (I think): a NAT Network. This may fail to work for you in the Owheo labs, and if it does, just continue with the exercises in the next section. It did work for me when I tested it.
...
:::warning
:warning:
Remember that your IP addresses may well be different from the ones I see, so you will need to replace the values I see with the values on your system.
:::
